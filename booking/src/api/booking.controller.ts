...
import { publishMessage, sendRpc } from '../amqp/queue';
...
const bookingController = {
  async bookEvent(req: Request, res: Response) {
    try {
      ...
      const eventData = await sendRpc(config.eventQueue!, { event: OutgoingEvents.eventInfo, data: rpcData });
      if (!eventData) {
      ...
      if ((eventData as Event).remaining_tickets > 0) {
        ...
        return res.status(200).json('you have been added to the waiting list');
      }
    } catch (err: any) {
